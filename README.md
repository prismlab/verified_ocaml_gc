...
* `original-runtime-changes-for-bdwgc-integration.diff` : This has the diff for changes in OCaml runtime needed to integrate `BDWGC`.
* `original-runtime-changes-for-verified-gc-integration.diff` : This has the
  diff for changes in OCaml runtime needed to integrate `verified-gc`. NOTE:
...
- **LowStar_Prims.c** : Handwritten code to define some operation for
  arithmetic, which is generated by F\*. (We don't want to link krml libraries,
  so we do this. )
