...
noextract inline_for_extraction
val heap_size : n:nat{n % Usize.v mword == 0 /\ n >=16 /\ n < 1099511627776}
...
let closinfo_val_from_closure_object (g:heap)
            ...
            :Tot (clos_info:Usize.t{ (is_hp_addr (Usize.add f_addr (mword))) /\
                                     (clos_info == read_word g (Usize.add f_addr (mword)))})=
 ...
 assert (clos_info == read_word g (Usize.add f_addr (mword)));
 clos_info
...
val closinfo_val_from_closure_object_lemma : (g:heap)->
                         ...
                         (requires (Usize.v f_addr >= Usize.v mword /\
                                   ...
                                   is_fields_within_limit1 (hd_address f_addr) (wosize_of_object1 (hd_address f_addr) g') /\
                                   read_word g (Usize.add f_addr (mword)) == read_word g' (Usize.add f_addr (mword))))
...
let clos_info = closinfo_val_from_closure_object g f_addr in
let start_env = extract_start_env_bits clos_info in
(*assert ((is_hp_addr (Usize.add f_addr (mword))) /\
           (clos_info == read_word g (Usize.add f_addr (mword))));*)
assert (start_env == Usize.shift_right (Usize.shift_left clos_info 8ul) 9ul);
assert (start_env == Usize.shift_right (Usize.shift_left (read_word g (Usize.add f_addr (mword))) 8ul) 9ul);
assert (Seq.length (objects2 0UL g) > 0);
...
(*closinfo_val1 gives assertion failure if (is_hp_addr (Usize.add f_addr (mword))) condition is not added before the result of post condition. Either we can
...
let start_env_clos_info_helper_lemma (g:heap{well_formed_heap2 g})
                              (f_addr:hp_addr{Usize.v f_addr >= Usize.v mword /\
                                  ...
                                  (Usize.v (wosize_of_object1 (hd_address f_addr) g) >= 2)})
                   : Lemma
                     (ensures ~(Seq.mem (Usize.add f_addr (mword)) (objects2 0UL g))) =
  ...
  let next_f_addr = Usize.add f_addr (mword) in
  let hdr_f_addr = hd_address f_addr in
...
let start_env_clos_info_helper_lemma1 (g:heap{well_formed_heap2 g})
                    ...
                    : Lemma
                      (requires is_hp_addr (Usize.add f_addr (mword)))
                      (ensures read_word g (Usize.add f_addr (mword)) == read_word (colorHeader1 v_id g c) (Usize.add f_addr (mword))) =
 let next_f_addr = Usize.add f_addr (mword) in
 let hdr_f_addr = hd_address f_addr in
 assert (Usize.v (wosize_of_object1 hdr_f_addr g) >= 2);
 let next_next_hdr_f_addr = Usize.add hdr_f_addr(Usize.mul 2UL mword) in
 ...
 //lemma_len_slice g (Usize.v (Usize.add next_f_addr (mword))) ((Usize.v (Usize.add next_f_addr (mword))) + (Usize.v mword));
...
let start_env_clos_info_lemma (g:heap{well_formed_heap2 g})
  ...
  lemma_len_slice g (Usize.v (Usize.add f_addr (mword))) ((Usize.v (Usize.add f_addr (mword))) + (Usize.v mword));
  lemma_len_slice g' (Usize.v (Usize.add f_addr (mword))) ((Usize.v (Usize.add f_addr (mword))) + (Usize.v mword));
  start_env_clos_info_helper_lemma1  g f_addr v_id c;
  assert (read_word g (Usize.add f_addr (mword)) == read_word g' (Usize.add f_addr (mword)));
